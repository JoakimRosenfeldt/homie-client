...
import { findThreadByName, INITIAL_MESSAGES, THREADS, type Thread } from "@/features/matches/data";
...
export default function MatchesScreen() {
  ...
  const newMatches = APPLICANTS.flatMap((applicant) => {
    if (!applicant.match) return [];

    const thread = findThreadByName(applicant.name);
    const hasSentMessage = thread
      ? (session.messages[thread.id] ?? []).some((message) => message.from === "me")
      : false;

    return thread && !hasSentMessage ? [{ applicant, thread }] : [];
  });
  ...
  return (
    <Screen contentStyle={{ gap: 0 }}>
      ...
      <ScrollView
        ...
        {newMatches.map(({ applicant, thread }) => (
          <Pressable
            key={applicant.id}
      ...
      <View style={{ borderTopWidth: 1, borderTopColor: theme.border }}>
        {THREADS.map((thread) => {
          const messages = session.messages[thread.id] ?? [];
          const latestMessage = messages[messages.length - 1]?.text ?? thread.last;
          const hasNewMessage = messages.length > (INITIAL_MESSAGES[thread.id]?.length ?? 0);
          ...
          return (
            <ThreadRow
              ...
              time={hasNewMessage ? "Now" : thread.time}
...
function ThreadRow({
  ...
  return (
    <Pressable
      accessibilityRole="button"
      accessibilityLabel={`${thread.name}. ${lastMessage}${unread ? ". Unread" : ""}`}
      onPress={onPress}
      style={({ pressed }) => ({
        flexDirection: "row",
        alignItems: "center",
      ...
      <View style={{ flex: 1, gap: 4, minWidth: 0 }}>
        <View style={{ flexDirection: "row", alignItems: "baseline", justifyContent: "space-between", gap: 8 }}>
          ...
          <Text style={{ fontSize: 11, fontWeight: "500", color: theme.faint }}>{time}</Text>
        </View>
